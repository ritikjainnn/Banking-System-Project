...
public class Withdrawl extends JFrame
{
	...
	ImageIcon but1 = new ImageIcon("Images\\with3.png");
	ImageIcon but2 = new ImageIcon("Images\\with4.png");
...
//		New Account Heading
		title =  new JLabel("<html><u>WITHDRAWL</u></html>");
		title.setBounds(1250, 180, 500, 40);
...
//		Bank Name
		...
		i_bank.add(lb_bname);
		lb_bname.setText(Login.bname.toUpperCase());
		lb_bname.setFont(f1);
...
//		Account Number
		...
		i_acc.add(lb_bacc);
		lb_bacc.setText(Login.accno.toUpperCase());
		lb_bacc.setFont(f1);
...
//		Button
	...
	void applyWithdrawl(float amount) throws Exception
	{
		float balance = 0;
		int sts = 0;
		...
		{	
			...
			if(sts == 1)
			{
				if(balance>=amount)
				{
					...
					st1.executeUpdate("update "+Login.bname+" set amnt = '"+balance+"' where accno = '"+Login.accno+"'");
					co1.close();
					JOptionPane.showMessageDialog(this, "Amount has been deducted.\nWe are redirecting to Menu Menu.");
					MainMenu m = new MainMenu();
					m.frame();
					super.dispose();
				}
			...
			{
				JOptionPane.showMessageDialog(this, "Your Account has been Freeze.\nWe are redirecting to Home Page.");
